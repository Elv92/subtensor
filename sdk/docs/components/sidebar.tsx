...
export function Sidebar({ tree }: { tree: TreeNode[] }) {
  ...
  return (
    <>
      {/* Desktop rail */}
      <aside className="max-md:hidden sticky top-14 h-[calc(100dvh-3.5rem)] w-64 shrink-0 overflow-y-auto bt-scroll border-e border-line py-6 pe-4">
        <nav>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div
            className="absolute inset-0 bg-fg/20 dark:bg-black/50"
            onClick={() => setOpen(false)}
          />
          <div className="absolute inset-y-0 start-0 w-72 overflow-y-auto bt-scroll bg-bg border-e border-line p-4">
            <button
              type="button"
