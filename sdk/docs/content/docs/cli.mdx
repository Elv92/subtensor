...
The `subtensor` command is the SDK with a shell in front: every read in the
catalog is `subtensor query <name>`, every transaction is
`subtensor tx <name>`, and both are generated from the same registries as the
...
```bash
subtensor config set network finney
subtensor config set wallet my_coldkey
subtensor config get
```
...
- `subtensor query <name>` — every [read](/docs/query); `query --help` lists
  them grouped by topic.
- `subtensor tx <name>` — every [transaction](/docs/tx); `tx --help` lists them
...
- `subtensor tools` — the JSON catalog of every transaction op with schemas,
  for agents.
