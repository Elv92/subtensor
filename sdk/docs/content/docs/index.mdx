...
description: Do anything on the Bittensor chain with the subtensor SDK and CLI — designed for agents and humans alike.
---
...
These docs cover the `subtensor` Python SDK and CLI: one install gives you a
library (`import subtensor`) and a command line (`subtensor`) that together can
...
This installs both the `subtensor` command and the `subtensor` Python package.
For the library alone (no terminal-UI dependencies), install plain `subtensor`.
...
- The same catalogs, live from the tool itself: `subtensor tools` on the CLI, `sub.intents.list_tools()` in Python.
