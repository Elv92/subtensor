...
```bash
subtensor tools        # JSON: every transaction op + summary, description, signer, input schema
```
...
On the CLI, every op in the catalog is `subtensor tx <op-name>` (underscores
become dashes) and every read is `subtensor query <name>`.
...
- Configuration comes from flags, `BT_*` environment variables, or
  `subtensor config` — highest to lowest precedence.
