...
OUT_DIR = Path(__file__).resolve().parent.parent / "subtensor" / "_generated"
...
RAW_ONLY: dict[str, set[str]] = {
    "SubtensorModule": {
        ...
        "swap_coldkey",
        "swap_hotkey_v2",
        # locks / liquidity-adjacent (leases are wrapped by intents)
        "lock_stake",
        "move_lock",
        "set_perpetual_lock",
        "set_reject_locked_alpha",
        # alpha burn/recycle + buyback variants
        "add_stake_burn",
        "burn_alpha",
        "recycle_alpha",
        ...
        "swap_stake_limit",
        "register_limit",
        # identity / metadata / misc (set_identity / set_subnet_identity are wrapped)
    ...
    "Proxy": {
        ...
        "proxy",
        # announced-proxy flow (non-zero delay) — not yet wrapped
        "announce",
        "proxy_announced",
        "reject_announcement",
        "remove_announcement",
        # pure (keyless) proxies and deposit maintenance — niche
        "create_pure",
        "kill_pure",
        "poke_deposit",
        "set_real_pays_fee",
...
def check_coverage() -> int:
    from subtensor._generated import calls as generated_calls
    from subtensor.intents import REGISTRY
