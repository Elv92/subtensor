...
Produces ``tests/fixtures/golden.json``: a corpus of byte-exact artifacts
...
from subtensor._transport.codec import strip_option_opaque_metadata
from subtensor._transport.interface import SubstrateConnection
from subtensor._transport.runtime_api import encode_runtime_api_params
from subtensor._transport.storage import storage_key
...
async def main() -> None:
    ...
    immortal = codec.signature_payload(
        transfer, era="00", nonce=7, tip=0, tip_asset_id=None,
        genesis_hash=genesis, era_block_hash=genesis,
    )
    payloads.append(
        {
    ...
    mortal = codec.signature_payload(
        transfer, era=dict(mortal_era), nonce=3, tip=0, tip_asset_id=None,
        genesis_hash=genesis, era_block_hash=birth_hash,
    )
    payloads.append(
        {
            "call_data_hex": "0x" + codec.call_data(transfer).hex(),
    ...
    for era, nonce in ((None, 0), ({"period": 64, "current": current}, 11)):
        ext = await sub.create_signed_extrinsic(
            transfer,
            ALICE,
