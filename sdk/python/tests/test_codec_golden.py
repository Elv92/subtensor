...
from subtensor._transport import codec as codec_mod
from subtensor._transport.codec import RuntimeCodec, strip_option_opaque_metadata

FIXTURE = Path(__file__).parent / "fixtures" / "golden.json"

pytestmark = pytest.mark.skipif(not FIXTURE.exists(), reason="golden fixture not recorded")
...
def test_metadata_ir_shape():
    ...
    subtensor = next(p for p in ir.pallets if p.name == "SubtensorModule")
    assert any(call.name == "add_stake" for call in subtensor.calls)
    assert any(e.name for e in subtensor.errors)
    assert "Tempo" in subtensor.storage_names
