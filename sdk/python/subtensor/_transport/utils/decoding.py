...
def _decode_scale_list_with_runtime(
    ...
    if runtime.implements_scaleinfo:
        ...
        obj = [scale_decode(x, y, runtime).value for (x, y) in zip(type_strings, scale_bytes_list)]
    return obj
...
def decode_query_map(
    ...
    for item in result_group_changes:
        ...
        pre_decoded_values.append(hex_to_bytes_(item[1]) if item[1] is not None else b"")
    ...
    for kts, vts, dk, dv in zip(
        ...
        try:
            ...
            if len(param_types) - len(params) == 1:
                ...
                try:
                    item_key = tuple(dk[i * 2 + 1] for i in range(len(param_types) - len(params)))
                except IndexError:
                    item_key = dk
