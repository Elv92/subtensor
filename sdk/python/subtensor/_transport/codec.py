...
class RuntimeCodec:
    ...
    def compose_call(self, module: str, function: str, params: dict) -> GenericCall:
        ...
        call.encode(
            {"call_module": module, "call_function": function, "call_args": params or {}}
        )
        return call
    ...
    def module_error(self, module_index: int, error_index: int) -> dict:
        ...
        error = self._metadata.get_module_error(
    ...
    def extrinsic_version(self) -> int:
        return self._metadata[1][1]["extrinsic"]["version"]
    ...
    def signature_payload(
        ...
        *,
        ...
        The field order follows the runtime's signed extensions from metadata.
        ...
        type_mapping: list[list[str]] = [["call", "CallBytes"]]
        if "CheckMortality" in signed_extensions:
            type_mapping.append(["era", signed_extensions["CheckMortality"]["extrinsic"]])
        if "CheckEra" in signed_extensions:
            type_mapping.append(["era", signed_extensions["CheckEra"]["extrinsic"]])
        if "CheckNonce" in signed_extensions:
            type_mapping.append(["nonce", signed_extensions["CheckNonce"]["extrinsic"]])
        ...
        if "CheckMetadataHash" in signed_extensions:
            type_mapping.append(["mode", signed_extensions["CheckMetadataHash"]["extrinsic"]])
        ...
        if "CheckEra" in signed_extensions:
            type_mapping.append(["block_hash", signed_extensions["CheckEra"]["additional_signed"]])
        if "CheckMetadataHash" in signed_extensions:
            type_mapping.append(
                ["metadata_hash", signed_extensions["CheckMetadataHash"]["additional_signed"]]
            )
        payload.type_mapping = type_mapping
        ...
        *,
        ...
        if self.extrinsic_version != 4:
            raise NotImplementedError(
                f"Extrinsic version {self.extrinsic_version} not supported"
            )
    ...
    def metadata_ir(self) -> MetadataIR:
        ...
        for pallet in self._metadata.pallets:
            calls = [
                CallIR(
                    name=call.name,
                    args=[
                        CallArgIR(name=arg["name"], type_name=arg.get("typeName"))
                        for arg in call.value.get("fields", [])
                    ],
            ...
            errors = [
                ErrorIR(name=error.name, docs=join_docs(error.docs))
                for error in (pallet.errors or [])
            ]
            storage_names = [
                item.value["name"]
            ...
            constant_names = [c.value["name"] for c in (pallet.constants or [])]
            pallets.append(
                PalletIR(
                    name=pallet.name,
                    index=pallet.value["index"],
                    calls=calls,
                    errors=errors,
        ...
        api_methods = [
            RuntimeApiMethodIR(api=api, method=method)
            for api, methods in self.runtime_api_map.items()
            for method in methods
        ]
        return MetadataIR(
            spec_version=self.spec_version, pallets=pallets, runtime_api_methods=api_methods
        )
