"""
The asyncio Substrate transport used by the Bittensor SDK: websocket RPC,
SCALE encoding/decoding, storage queries, and extrinsic submission. The
docstring for the `AsyncSubstrateInterface` class goes more in-depth in
...
from .utils.cache import cached_fetcher
...
from .type_registry import _TYPE_REGISTRY

ResultHandler = Callable[[dict, Any], Awaitable[tuple[dict, bool]]]

logger = logging.getLogger("subtensor.transport")
raw_websocket_logger = logging.getLogger("subtensor.transport.raw_websocket")
...
class AsyncExtrinsicReceipt:
    ...
    async def process_events(self):
        if await self.triggered_events:
            ...
            self.__total_fee_amount, has_transaction_fee_paid_event = extract_total_fee_amount(
                events
            )
            ...
            for event in events:
                if is_extrinsic_success_event(event):
                    ...
                    self.__total_fee_amount += extract_fallback_deposit_fee_amount(event)
    ...
    async def _resolve_module_error_message(self, module_index: int, error_index: int) -> dict:
        if self.block_hash:
...
class Websocket:
    ...
    async def _cancel(self):
        try:
            ...
            logger.warning(f"{e} encountered while trying to close websocket connection.")
        try:
    ...
    async def _connect_internal(self, force):
        ...
        if self.state not in (State.OPEN, State.CONNECTING) or force:
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(loop.sock_connect(tcp_sock, sockaddr), timeout=10.0)
                except Exception:
                    tcp_sock.close()
    ...
    async def _handler(self, ws: ClientConnection) -> Optional[Exception]:
        ...
        if should_reconnect is True:
            if len(self._received_subscriptions) > 0:
                logger.warning(
                    "Connection lost with %d open subscription(s); "
                    "the in-flight request cannot be resumed.",
                    len(self._received_subscriptions),
                )
                async with self._lock:
                    self._received_subscriptions.clear()
            ...
            async with self._lock:
                ...
                for original_id in list(self._inflight.keys()):
                    ...
                    if isinstance(parsed, list):
                        ...
                        logger.debug(f"Resubmitting batch {[sub['id'] for sub in parsed]}")
...
class AsyncSubstrateInterface(SubstrateMixin):
    ...
    def __init__(
        ...
        *,
        ...
        """
        The asyncio-compatible version of the subtensor interface commands we use in subtensor. It is important to
        ...
        Args:
            ...
            _ssl_context: optional session-resuming SSL context enabling TLS session
                reuse across reconnections.
    ...
    async def properties(self):
        if self._properties is None:
            self._properties = (await self.rpc_request("system_properties", [])).get("result")
    ...
    async def get_storage_item(
        ...
        if metadata_pallet is None:
            raise StorageFunctionNotFound(f"Metadata pallet not found for module '{module}'")
        storage_item = metadata_pallet.get_storage_function(storage_function)
        return storage_item

    async def _load_registry_at_block(self, block_hash: Optional[str], runtime_config=None):
        ...
        metadata_option_bytes = bytes.fromhex(metadata_option_hex_str.removeprefix("0x"))
    ...
    async def _get_runtime_for_version(
        ...
        runtime_config.update_type_registry(load_type_registry_preset(name="core") or {})
        ...
        if metadata_v15 is not None:
            ...
            logger.debug(f"Retrieved metadata v15 for {runtime_version} from Substrate node")
        else:
            metadata = await self.get_block_metadata(
                block_hash=runtime_block_hash, runtime_config=runtime_config
            )
        if metadata is None:
            raise SubstrateRequestException(f"No metadata for block '{runtime_block_hash}'")
        runtime = Runtime(
            chain=self.chain,
            metadata=metadata,
    ...
    async def subscribe_storage(
        ...
        async def result_handler(message: dict, subscription_id: str) -> tuple[bool, Optional[Any]]:
            result_found = False
    ...
    async def _get_block_handler(
                ...
                for idx, log_data in enumerate(block_data["header"]["digest"]["logs"]):
                    if isinstance(log_data, str):
                        ...
                        try:
                            ...
                            log_digest.decode(
                                check_remaining=self.config.get("strict_scale_decode") or False
                            )
                            ...
                            if include_author and "PreRuntime" in log_digest.value:
                                if runtime.implements_scaleinfo:
                                    ...
                                    if engine == b"BABE":
                                        babe_predigest = runtime.runtime_config.create_scale_object(
                                        ...
                                        aura_predigest = runtime.runtime_config.create_scale_object(
                                        ...
                                        assert validator_set is not None
                                        rank_validator = aura_predigest.value["slot_number"] % len(
                                            validator_set
                                        )
        ...
        if callable(subscription_handler):
            ...
            async def result_handler(message: dict, subscription_id: str) -> tuple[Any, bool]:
                reached = False
                subscription_result = None
                if "params" in message:
            ...
            if header_only:
                response = await self.rpc_request("chain_getHeader", [block_hash], runtime=runtime)
                return await decode_block(
                    {"header": response["result"]}, block_data_hash=block_hash
                )

            else:
                response = await self.rpc_request("chain_getBlock", [block_hash], runtime=runtime)
    ...
    async def _preprocess(
        ...
        queryable = (
            str(query_for) if query_for is not None else f"{method}{random.randint(0, 7000)}"
        )
        return Preprocessed(
            queryable,
            method,
            ...
            try:
                ...
                while True:
                    for item_id in request_manager.unresponded():
                        if item_id not in request_manager.responses or inspect.iscoroutinefunction(
                            result_handler
                        ):
                            if response := await ws.retrieve(item_id):
                                ...
                                if result_processor is not None and not inspect.iscoroutinefunction(
                                    result_handler
                                ):
    ...
    async def rpc_request(
        ...
        if "error" in result[payload_id][0]:
            if "Failed to get runtime version" in (
                ...
                logger.warning("Failed to get runtime. Re-fetching from chain, and retrying.")
                runtime = await self.init_runtime(block_hash=block_hash)
                return await self.rpc_request(
                    method,
                    params,
                ...
                "Client error: Api called for an unknown Block: State already discarded" in err_msg
            ):
    ...
    async def _cached_get_block_hash(self, block_id: int) -> str:
        """Separated from `get_block_hash` so the cache layer can be swapped if needed."""
    ...
    async def create_scale_object(
        ...
        **kwargs,
        ...
        return runtime.runtime_config.create_scale_object(type_string, data=data, **kwargs)
    ...
    async def generate_signature_payload(
        ...
        signature_payload = runtime.runtime_config.create_scale_object("ExtrinsicPayloadValue")
    ...
    async def _do_runtime_call_old(
        ...
        _decoded = await self.decode_scale("Vec<u8>", result_vec_u8_bytes, runtime=runtime)
    ...
    async def runtime_call(
        ...
        try:
            if runtime.metadata_v15 is None:
                _ = runtime.runtime_config.type_registry["runtime_api"][api]["methods"][method]
                runtime_api_types = runtime.runtime_config.type_registry["runtime_api"][api].get(
                    "types", {}
                )
    ...
    async def get_account_nonce(self, account_address: str) -> int:
        ...
        if await self.supports_rpc_method("state_call"):
            nonce = await self.runtime_call("AccountNonceApi", "account_nonce", [account_address])
    ...
    async def get_account_next_index(self, account_address: str, use_cache: bool = True) -> int:
        """
    ...
    async def get_payment_info(
        ...
        if nonce is None:
            nonce = await self.get_account_next_index(keypair.ss58_address, use_cache=False)
    ...
    async def get_type_definition(self, type_string: str, block_hash: Optional[str] = None) -> str:
        """
    ...
    async def query(
        ...
        payload = [
            self.make_payload(preprocessed.queryable, preprocessed.method, preprocessed.params)
        ]
    ...
    async def query_map(
        ...
        if len(result_keys) > 0:
            ...
            if not fully_exhaust:
                ...
                page_batches = [
                    result_keys[i : i + page_size] for i in range(0, len(result_keys), page_size)
                ]
                payloads = [
                    self.make_payload(str(idx), "state_queryStorageAt", [page_batch, block_hash])
    ...
    async def create_multisig_extrinsic(
        ...
        # No prior approvals -> storage is empty (None); this is the first signer.
        multisig_details = multisig_details_.value or {}
    ...
    async def submit_extrinsic(
        ...
        async def result_handler(message: dict, subscription_id) -> tuple[dict, bool]:
            ...
            if "params" in message and isinstance(message["params"]["result"], (dict, str)):
                ...
                if "usurped" in message_result:
                    failure_message = f"Subscription {subscription_id} usurped: {message_result}"
                if "retracted" in message_result:
                    failure_message = f"Subscription {subscription_id} retracted: {message_result}"
                if "finalitytimeout" in message_result:
                    failure_message = (
                        f"Subscription {subscription_id} finalityTimeout: {message_result}"
                    )
                if "dropped" in message_result:
                    failure_message = f"Subscription {subscription_id} dropped: {message_result}"
                if "invalid" in message_result:
                    failure_message = f"Subscription {subscription_id} invalid: {message_result}"
                ...
                if "finalized" in message_result and wait_for_finalization:
                    ...
                    "inblock" in message_result and wait_for_inclusion and not wait_for_finalization
                ):
                    logger.debug("Extrinsic included. Unsubscribing.")
                    async with self.ws as ws:
                        await ws.unsubscribe(subscription_id)
                    return {
                        "block_hash": message_result.get("inblock", message_result.get("inBlock")),
                        "extrinsic_hash": "0x{}".format(extrinsic.extrinsic_hash.hex()),
                        "finalized": False,
                    }, True
        ...
        if wait_for_inclusion or wait_for_finalization:
            ...
            result = AsyncExtrinsicReceipt(substrate=self, extrinsic_hash=response["result"])
    ...
    async def get_metadata_call_function(
        ...
        return self._get_metadata_call_function(module_name, call_function_name, runtime)
    ...
    async def get_metadata_event(self, module_name, event_name, block_hash=None) -> Optional[Any]:
        """
    ...
    async def _cached_get_block_number(self, block_hash: str) -> int:
        """Separated from `get_block_number` so the cache layer can be swapped if needed."""
    ...
    async def wait_for_block(
        ...
        async def _handler(block_data: dict[str, Any]):
            ...
            if number >= required_number:
                return r if (r := await result_handler(block_data)) is not None else True
            return None

        args = inspect.getfullargspec(result_handler).args
        if len(args) != 1:
            raise ValueError("result_handler must take exactly one arg: the dict block data.")

        assert self.last_block_hash is not None
        co = self._get_block_handler(self.last_block_hash, subscription_handler=_handler)
