    ...
    def retrieve(
        ...
        if block is not None:
            ...
            with suppress(AttributeError):
                runtime = self.versions.get(self.block_hashes.get(self.blocks.get(block)))
                self.last_used = runtime
                return runtime
        if block_hash is not None:
            if runtime_version is not None:
...
def _decode_v15_metadata(
    ...
    if runtime_config is None:
        ...
        rc = runtime_config
    meta_obj = rc.create_scale_object("MetadataVersioned", data=ScaleBytes(raw_metadata_bytes))
    meta_obj.decode()
    return meta_obj
...
class Runtime:
    ...
    def __setstate__(self, state):
        ...
        self.runtime_config.update_type_registry(load_type_registry_preset(name="core") or {})
        ...
        if type_registry_preset_dict:
            ...
            if self.implements_scaleinfo is False:
                ...
                self.runtime_config.update_type_registry(
                    load_type_registry_preset("legacy", use_remote_preset=use_remote_preset) or {}
                )
...
class RequestManager:
    def __init__(self, payloads):
        self.response_map = {}
        self.responses = defaultdict(lambda: {"complete": False, "results": deque(maxlen=100)})
    ...
    def get_results(self) -> RequestResults:
        ...
        """
        return {request_id: info["results"] for request_id, info in self.responses.items()}
...
class SubstrateMixin(ABC):
    ...
    def _runtime_config_copy(self, implements_scale_info: bool = False):
        ...
        runtime_config.active_spec_version_id = self.runtime_config.active_spec_version_id
    ...
    def ss58_encode(self, public_key: str | bytes, ss58_format: Optional[int] = SS58_FORMAT) -> str:
        """
    ...
    def serialize_module_event(module, event, spec_version, event_index: str) -> dict:
        ...
        return {
            ...
            "event_args": [
                {"event_arg_index": idx, "type": arg} for idx, arg in enumerate(event.args)
            ],
    ...
    def extension_call(self, name, **kwargs):
        raise NotImplementedError("Extensions not implemented in AsyncSubstrateInterface")
    ...
    def _encode_scale(self, type_string, value: Any, runtime: Optional[Runtime] = None) -> bytes:
        """
        ...
        if isinstance(value, ScaleType):
            ...
            value = value.value
        return bytes(runtime.runtime_config.create_scale_object(type_string).encode(value).data)
    ...
    def _get_metadata_events(self, runtime: Runtime) -> list[dict]:
        ...
        for event_index, (module, event) in runtime.metadata.event_index.items():
            event_list.append(
                self.serialize_module_event(module, event, runtime.runtime_version, event_index)
            )
    ...
    def _get_metadata_runtime_call_function(
        ...
        """
        try:
            runtime_call_def = runtime.runtime_config.type_registry["runtime_api"][api]["methods"][
                method
            ]
        ...
        runtime_call_def_obj = runtime.runtime_config.create_scale_object("RuntimeCallDefinition")
