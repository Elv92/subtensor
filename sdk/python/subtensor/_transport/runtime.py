...
def _disk_cache_dir() -> Path:
    override = os.getenv("SUBTENSOR_RUNTIME_CACHE_DIR")
    if override:
...
class RuntimeManager:
    ...
    def note_runtime_upgrade(self) -> None:
        """Drop the cached head codec (call when a decode hints at a new runtime)."""
        self._head_codec = None
        self._head_checked_at = float("-inf")
    ...
    async def _codec_for_block_hash(self, block_hash: str) -> RuntimeCodec:
        ...
        if runtime_info is None:
            raise SubstrateRequestException(
                {"error": {"message": f"No runtime information for block '{block_hash}'"}}
            )
        spec_version = runtime_info["specVersion"]
    ...
    async def _parent_hash(self, block_hash: str) -> str:
        ...
        if header is None:
            raise SubstrateRequestException(
                {"error": {"message": f'Block not found for "{block_hash}"'}}
            )
    ...
    async def _fetch_codec(
        self, spec_version: int, transaction_version: int, at_hash: str
    ) -> RuntimeCodec:
    ...
    async def _build_codec(
        ...
        if cached is not None:
            ...
            logger.debug(f"Runtime {spec_version}: metadata loaded from disk cache")
        else:
            metadata_bytes, is_v15 = await self._download_metadata(at_hash)
            tx_version = transaction_version
            _save_metadata_to_disk(genesis, spec_version, metadata_bytes, tx_version, is_v15)
            logger.debug(
    ...
    async def _download_metadata(self, at_hash: str) -> tuple[bytes, bool]:
        ...
        if not legacy:
            raise SubstrateRequestException(
                {"error": {"message": f"No metadata for block '{at_hash}'"}}
            )
