...
from .contract import BlockData, InclusionReport, MetadataIR, MultisigAccount, SignedExtrinsic
from .errors import BlockNotFound, SubstrateRequestException
from .extrinsics import IMMORTAL, NonceCache, resolve_outcome
...
class SubstrateConnection:
    ...
    async def query(
        ...
        raw = await self._session.request(
            "state_getStorageAt", ["0x" + key.hex(), block_hash]
        )
    ...
    async def query_batch(
        ...
        response = await self._session.request(
            "state_queryStorageAt", [key_hexes, block_hash]
        )
        by_key: dict[str, Optional[bytes]] = {}
        for group in response or []:
            for key_hex, value_hex in group["changes"]:
                by_key[key_hex] = (
                    bytes.fromhex(value_hex[2:]) if value_hex is not None else None
                )
    ...
    async def get_constant(
        self, module: str, name: str, block_hash: Optional[str] = None
    ) -> Any:
        codec = await self._runtimes.codec_at(block_hash)
    ...
    async def create_signed_extrinsic(
        ...
        *,
        ...
        """Sign a composed call at the current runtime.

        With no ``nonce`` the account's next index is taken (and cached for
        pipelining); an explicit nonce is pinned into the cache unless
        ``_skip_cache_update`` (fee estimation signs without intent to submit).
        """
        codec = await self._runtimes.codec_at(None)
        if nonce is None:
        ...
        *,
        ...
        if not (wait_for_inclusion or wait_for_finalization):
            result = await self._session.request(
                "author_submitExtrinsic", [extrinsic.data_hex]
            )
        ...
        try:
            async for update in subscription:
                ...
                for bad in ("usurped", "retracted", "finalitytimeout", "dropped", "invalid"):
                    if bad in normalized:
                        raise SubstrateRequestException(
                            {
                                "error": {
                                    "message": f"Extrinsic {extrinsic.extrinsic_hash} {bad}: "
                                    f"{status}"
                                }
                            }
                        )
                if "finalized" in normalized and wait_for_finalization:
                    block_hash = normalized["finalized"]
                    finalized = True
                    break
                if (
                    "inblock" in normalized
                    and wait_for_inclusion
                    and not wait_for_finalization
                ):
                    block_hash = normalized["inblock"]
                    break
        finally:
            await subscription.unsubscribe()
    ...
    def generate_multisig_account(
        self, signatories: list[str], threshold: int
    ) -> MultisigAccount:
