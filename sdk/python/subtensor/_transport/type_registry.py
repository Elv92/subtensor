...
def _cyscale_decode(type_name: str, raw_bytes: bytes, runtime: "Runtime") -> Any:
    ...
    return runtime.runtime_config.batch_decode([f"scale_info::{type_id}"], [raw_bytes])[0]
...
def _encode_vec_u8(params, runtime: "Runtime") -> bytes:
    ...
    return bytes(runtime.runtime_config.create_scale_object("Vec<u8>").encode(list(raw)).data)
...
def _encode_vec_vec_u8(params, runtime: "Runtime") -> bytes:
    ...
    return bytes(
        runtime.runtime_config.create_scale_object("Vec<Vec<u8>>").encode(preprocessed).data
    )
