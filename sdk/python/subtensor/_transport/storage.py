...
def _hasher_for(entry: StorageEntry, index: int):
    name = entry.param_hashers[index] if index < len(entry.param_hashers) else None
    try:
...
def decode_storage_value(
    codec: RuntimeCodec, entry: StorageEntry, raw: Optional[bytes]
) -> Any:
...
def _concat_len(entry: StorageEntry, index: int) -> int:
    hasher = entry.param_hashers[index] if index < len(entry.param_hashers) else "Twox128"
    try:
