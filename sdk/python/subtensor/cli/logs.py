...
The SDK emits diagnostics through stdlib loggers under ``subtensor.*`` and
...
``SUBTENSOR_LOG`` (error/warning/info/debug/trace) overrides the flags, which
...
_RAW_WEBSOCKET_LOGGER = "subtensor.transport.raw_websocket"
...
def setup_logging(verbosity: int = 0, quiet: bool = False) -> None:
    ...
    root = logging.getLogger("subtensor")
    root.setLevel(level)
...
def _replace_handler(target: logging.Logger, handler: logging.Handler) -> None:
    ...
    handler.set_name("subtensor-cli")
    target.addHandler(handler)


def _remove_our_handlers(target: logging.Logger) -> None:
    for existing in list(target.handlers):
        if existing.get_name() == "subtensor-cli":
