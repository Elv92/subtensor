...
from .output import Output, STYLE_COMMAND, STYLE_HINT, STYLE_KEY, STYLE_NAME
...
def _pick(
    ...
    if not positions:
        ...
        app_ctx.output.error(
            ...
            help="`subtensor stake list` shows every position",
        )
        raise typer.Exit(1)
...
def _stake_owner(app_ctx: AppContext, kwargs: dict) -> tuple[str, str]:
    ...
    try:
        ...
        app_ctx.output.error(
            f"wallet {app_ctx.wallet_name!r} has no usable coldkey: {error}"
        )
        raise typer.Exit(1)
    ...
    if per_position:
        ...
        for hotkey, group in by_hotkey.items():
            ...
            if len(group) == 1:
                only = group[0]
                rows.append(
                    _Row(hotkey, only.netuid, subnet_ref(f"netuid {only.netuid}"), str(only.stake), value, only.netuid != 0)
                )
            else:
                rows.append(
                    _Row(hotkey, None, f"{len(group)} subnets", f"≈ {value}", value, False)
                )
...
def _print_rows(
    ...
    hint.append("  ")
    hint.append(
        "Where the stake comes from — pick a position below.", style=STYLE_HINT
    )
    console.print(hint)
...
def _ask_row(
    ...
    *,
    ...
    while True:
        ...
        console.print(
            "  [dim]note: no stake found on that hotkey — using it anyway[/dim]"
        )
