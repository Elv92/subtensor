...
def sudo_set(
    ...
    `subtensor sudo get` to see current values and per-parameter help.
    """
    ...
    try:
        ...
        app_ctx.output.error(
            str(error),
            help=f"`subtensor sudo get --netuid {netuid} --name {name}` explains "
...
def _prompt_take(app_ctx: AppContext, hotkey: str) -> int:
    ...
    def _parse(app_ctx: AppContext, raw: str) -> int:
        ...
        if record is not None and not (
            record["min"] <= value / U16_MAX <= record["max"]
        ):
            raise ValueError(
...
def start_subnet(
    ...
    post-registration waiting period (see `subtensor sudo check-start`).
...
def stake_burn(
    ...
    limit_price: int = typer.Option(
        ..., "--limit-price", help=StakeBurn.field_help("limit_price")
    ),
    hotkey_ss58: Optional[str] = typer.Option(
        None, address_cli_name("hotkey_ss58"), help=ss58_param_help("hotkey_ss58")
    ),
