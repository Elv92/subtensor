"""`subtensor timelock`: seal data that anyone can open at a known future time.
...
def _reveal_note(sealed: Timelocked) -> str:
    ...
    return f"reveals at {when} (round {sealed.reveal_round}, in {format_duration(sealed.remaining)})"
...
def encrypt_cmd(
    ...
    # A bare `subtensor timelock encrypt` asks for what's missing (terminal
