"""`subtensor multisig`: named multisig signer sets and pending operations.

Saved signer sets are reused by `subtensor call --multisig NAME` and
`subtensor multisig pending`. Entries live in the local multisig book
(see subtensor/config.py).
"""
...
@app.command(
    "add",
    epilog=(
        "Example: subtensor multisig add finney-sudo --threshold 2 "
...
@app.command(
    "pending",
    epilog=(
        "Example: subtensor multisig pending --multisig finney-sudo "
...
def multisig_pending(
    ...
    if multisig is None and not (signatories or other_signatories):
        confirm_wallet(
            app_ctx, help_text="Multisig wallet name (-w name).", must_exist=False
        )
    ...
    if threshold is None:
        ...
        if saved:
            ...
            help_text = (
                f"save one with `subtensor multisig add {app_ctx.wallet_name} "
