...
def stake_list(
    ...
    show_dust: bool = typer.Option(
        False,
        "--dust",
        help="Also show dust subnets and positions (spot value < τ0.001). JSON always includes every position.",
    ),
):
...
def child_revoke(
    ...
    weight. Subject to the same rate limit as `subtensor stake child set`.
    """
    app_ctx: AppContext = ctx_of(ctx)
...
def stake_wizard(ctx: typer.Context):
    """Interactive stake movement guide (use `subtensor stake move`, `transfer`, or `swap`)."""
    app_ctx: AppContext = ctx_of(ctx)
    app_ctx.output.detail(
        "stake wizard",
        {
            "move": "subtensor stake move --origin-netuid N --destination-netuid M --amount-alpha X",
            "transfer": "subtensor stake transfer --origin-hotkey H1 --destination-hotkey H2 ...",
            "swap": "subtensor stake swap --origin-netuid N --destination-netuid M --amount-alpha X",
