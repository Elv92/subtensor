"""`subtensor proxy`: on-chain proxy account management and the local proxy book."""
...
def create_proxy(
    ...
    `subtensor proxy kill NAME` work later.
    """
    app_ctx: AppContext = ctx_of(ctx)
    result = app_ctx.submit(
        CreatePureProxy(proxy_type=proxy_type.value, delay=delay, index=index)
    )
    ...
    if not pure:
        if name:
            app_ctx.output.message(
                ...
                "add it manually with `subtensor proxy book add`"
            )
        return
    if name:
        entry: dict[str, Any] = {
        ...
        app_ctx.output.message(
            ...
            f"`subtensor proxy book add --name NAME --address {pure}` "
...
def add_proxy(
    ...
    app_ctx.submit(
        AddProxy(delegate_ss58=delegate, proxy_type=proxy_type.value, delay=delay)
    )
...
def remove_proxy(
    ...
    delegate = app_ctx.resolve_address("coldkey_ss58", delegate_ss58)
    app_ctx.submit(
        RemoveProxy(delegate_ss58=delegate, proxy_type=proxy_type.value, delay=delay)
    )
...
def kill_proxy(
    ...
    proxy book entry created via `subtensor proxy create --name`, all of them
    ...
    if name and entry is None:
        app_ctx.output.error(
            ...
            help="`subtensor proxy book list` shows saved entries",
        )
        raise typer.Exit(1)
    ...
    if height_value is None or ext_value is None:
        app_ctx.output.error(
            ...
            note="the chain identifies a pure proxy by the exact create_pure call "
            "that spawned it",
            help="create with `subtensor proxy create --name NAME` to record them, or "
...
def book_list(
    ...
    if verify and entries:
        ...
        entries = [
            {**entry, "status": status} for entry, status in zip(entries, statuses)
        ]
    out = app_ctx.output
    if out.json_mode:
