...
def hyperparameters(
    ...
    name: Optional[str] = typer.Option(
        None,
        "--name",
        help="Explain a single hyperparameter in detail, including the command "
        "that changes it.",
    ),
):
...
def subnet_conviction(
    ...
    if now >= changeable_at:
        ...
        age_note = f"~{days_away:.0f}d away — a subnet must be one year old before ownership can change"
    parameters = [
        (
            "alpha out",
            str(data["alpha_out"]),
        ...
        (
            ...
            f"conviction closes half its gap to the locked mass every ~{_halflife(data['maturity_rate'])}",
        ),
    ...
    explanation = [
        f"Anyone can lock alpha to a hotkey with `subtensor lock add --netuid {netuid} "
        ...
        f"rises and then fades away; a perpetual lock (`subtensor lock mode --netuid {netuid} "
...
def register_subnet(
    ...
    it first with `subtensor subnets burn-cost`.
    """
    app_ctx: AppContext = ctx_of(ctx)
...
def create_subnet(ctx: typer.Context):
    ...
    subnet registration cost in TAO (see `subtensor subnets create-cost`).
    The subnet does not emit until its owner activates it with
    `subtensor sudo start`.
    """
    app_ctx: AppContext = ctx_of(ctx)
