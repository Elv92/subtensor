...
def convert(
    ...
    if from_rao:
        ...
        app_ctx.output.detail(
            None,
            {"rao": int(amount), "amount": value.amount, "currency": currency, "display": str(value)},
        )
    else:
...
def latency(
    ...
    async def _probe(label: str, endpoint: str) -> dict:
        ...
        try:
            ...
            result = {"network": label, "endpoint": endpoint, "latency_ms": round(ms, 1), "ok": True}
    ...
    animate = (
        not app_ctx.output.json_mode and not app_ctx.output.quiet and sys.stderr.isatty()
    )
    if animate:
