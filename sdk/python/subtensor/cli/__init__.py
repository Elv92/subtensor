"""The ``subtensor`` command-line interface.

Built entirely on the SDK's public surface (``subtensor.Client`` and friends);
...
try:
    ...
    raise ModuleNotFoundError(
        f"the subtensor CLI needs the optional CLI dependencies ({_error.name} "
        "is not installed); install them with: pip install 'subtensor[cli]'"
    ) from _error
