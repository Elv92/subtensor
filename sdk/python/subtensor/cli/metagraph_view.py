...
def show_metagraph(app_ctx: AppContext, netuid: int, graph: Any) -> None:
    ...
    app_ctx.output.metagraph(
        ...
        hint=(
            ...
            f"`subtensor subnets hyperparameters {netuid}` explains each parameter"
        ),
    )
...
def _sections(app_ctx: AppContext, graph: dict, netuid: int) -> list[tuple[Optional[str], list[Row]]]:
    def tao(key: str) -> str:
