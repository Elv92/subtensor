...
def explain(
    ...
    if code is None:
        ...
        out.message(
            "explain one entry with `subtensor explain <Name>` or "
            "`subtensor explain <code>`; filter with "
            "`subtensor explain --chain <pallet>`"
        )
        return
    query = code.strip()
    try:
        ...
        if not matches:
            out.error(
                ...
                help="run `subtensor explain` for the semantic codes, or "
                "`subtensor explain --chain` for the chain error catalog",
            )
            raise typer.Exit(1)
