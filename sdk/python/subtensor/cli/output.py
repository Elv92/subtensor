...
_NETUID_REF_RE = re.compile(
    r"\b(?:netuid|subnet)\s+(?P<id>\d+)(?:\s+\((?P<name>[^()]{1,64})\))?"
)
...
def _address_kind_for_key(key: str) -> Optional[str]:
    ...
    if "hotkey" in key:
        return "hotkey"
    if "coldkey" in key or key.endswith("ss58") or key in _ADDRESS_KEYS | {"dest", "wallet", "owner"}:
        return "coldkey"
    return None
...
class Output:
    def __init__(
        self, json_mode: bool = False, quiet: bool = False, network: Optional[str] = None
    ):
        ...
        self.token_symbols: dict[int, str] = (
            cfg.load_token_symbols(network) if network else {}
        )
    ...
    def update_subnet_names(self, names: dict[int, str]) -> None:
        ...
        if self.network:
            try:
                ...
                pass  # the cache is cosmetic; never fail a command over it
    ...
    def _linked_text(
        self, value: str, style: str, kind: Optional[str] = None
    ) -> Text:
    ...
    def transfer_history(self, title: str, records: list[dict[str, Any]]) -> None:
        ...
        rows = [
            (record, f"{'−' if record['direction'] == 'out' else '+'}"
        ...
        for record, amount_text in rows:
            ...
            line.append("  ")
            url = self.extrinsic_url(
                f"{record['block_number']}-{record['extrinsic_idx']:04}"
            )
            line.append(
            ...
            line.append(" ")
            counterparty = (
                record["to"] if record["direction"] == "out" else record["from"]
            )
            ...
            if name:
                ...
                line.append(
                    counterparty, style=f"link {account_url}" if account_url else ""
                )
            if not record["success"]:
        ...
        *,
        ...
        for row, cell in zip(rows, subnet_cells):
            ...
            for i, key in enumerate(keys):
                value = str(row[key])
                line.append(
                    value.rjust(widths[i]), style="dim" if value == "—" else ""
                )
                line.append("  ")
            self._out.print(line, soft_wrap=True)
        if footer:
    ...
    def metagraph(
        ...
        for header, rows in sections:
            ...
            for key, value, note in rows:
                ...
                line.append_text(
                    self._linked_text(
                        value, _value_style(key, value), _address_kind_for_key(key)
                    )
                )
                if note:
                    line.append(f"  {note}", style=STYLE_HINT)
    ...
    def _multisig_node(
        self, name: str, ss58: str | None, *, threshold: int, count: int
    ) -> Text:
        node = Text()
    ...
    def wallet_list(
        ...
        *,
        ...
        if proxies:
            ...
            coldkey_names = {
                ck.get("ss58"): ck["coldkey"] for ck in records if ck.get("ss58")
            }
            self._out.print()
    ...
    def multisig_followup(
        self, followup: dict[str, Any], *, suppress_decode: bool = False
    ) -> None:
        ...
        for key, value in fields.items():
            ...
            if key in ("approved_by", "needs"):
                ...
                for index, item in enumerate(value):
                    self._kv_line(key if index == 0 else "", width, self._named_label_text(str(item)))
                continue
    ...
    def _print_failure(self, result: ExtrinsicResult) -> None:
        ...
        long-form explanation stays behind `subtensor explain <code>`."""
        error = result.error
        ...
        if explain_target:
            tail = _prose(
                f"for more information about this error, run "
                f"`subtensor explain {explain_target}`"
            )
    ...
    def explain_chain(self, matches: list[dict[str, str]]) -> None:
        ...
        for code in dict.fromkeys(match["code"] for match in matches):
            tail = _prose(
                f"for more information about this code, run `subtensor explain {code}`"
            )
            tail.style = STYLE_HINT
            self._out.print(tail)
