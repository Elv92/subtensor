"""Thin conveniences over ``bittensor_wallet``.

Key management lives in the ``bittensor-wallet`` package. This module only adds
small helpers for the common create / regenerate / list flows so callers don't
have to reach for the wallet library directly for basic tasks.
"""
...
from bittensor_wallet.keyfile import get_password_from_environment
from bittensor_wallet.keypair import CRYPTO_ED25519, CRYPTO_SR25519
...
def resolve_wallet_password(
    ...
    *,
    ...
    3. per-wallet env from ``bittensor_wallet`` (``BT_PW__...``)
    ...
    7. ``None`` — ``bittensor_wallet`` prompts interactively if the key is encrypted
    """
    if password:
        return password
    ...
    if env_password:
        return env_password
    native = get_password_from_environment(wallet.coldkey_file.env_var_name())
    if native:
        return native
    ...
    if keychain:
        ...
        if not macos_prompt:
            raise ValueError(
                ...
                "run `subtensor wallet keychain save -w "
                f"{wallet.name}`"
            )
    if macos_prompt:
...
def signing_keypair(
    ...
    *,
    ...
    try:
        ...
        msg = str(error)
        if "Wrong password" in msg or "Decryption error" in msg:
            if macos_prompt and not keychain:
                hint = (
                    f" The password entered for wallet {wallet.name!r} did not decrypt its coldkey. "
                    "`--macos-password` only prompts — it does not set or change the wallet password. "
                    "Use the password from when the coldkey was created. After a successful unlock, "
                    f"run `subtensor wallet keychain save -w {wallet.name}` and use `--keychain-password`."
                )
            elif keychain:
                hint = (
                    f" Keychain entry for wallet {wallet.name!r} did not decrypt its coldkey. "
                    f"Re-save with `subtensor wallet keychain save -w {wallet.name}`."
                )
            else:
                hint = (
                    " Set BT_WALLET_PASSWORD, use --keychain-password / --macos-password, "
                    "or run via `doppler run -- subtensor ...`."
                )
        else:
            hint = (
                " Set BT_WALLET_PASSWORD, use --keychain-password / --macos-password, "
        ...
        raise ValueError(f"could not unlock coldkey: {error}.{hint}") from error
