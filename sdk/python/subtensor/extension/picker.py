...
def _select_account(
    ...
    *,
    ...
    if len(filtered) > 1:
        hint = _format_account_hint(filtered)
        raise BridgeError(
            "multiple extension accounts matched; pass --signer-address to choose one: "
            f"{hint}"
        )
    return filtered[0]
...
def _format_account_hint(accounts: list[ExtensionAccount]) -> str:
    return ", ".join(f"{account.name} ({account.address}, {account.source})" for account in accounts)
