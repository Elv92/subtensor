...
REMEDIATION: dict[ErrorCode, str] = {
    ErrorCode.INSUFFICIENT_BALANCE: (
        "fund the signing account or reduce the amount; check with `subtensor wallet balance`"
    ),
    ...
    ErrorCode.NOT_REGISTERED: (
        "register the hotkey on this subnet first with `subtensor subnets register`"
    ),
    ...
    ErrorCode.SUBNET_NOT_EXISTS: (
        "use an existing netuid; `subtensor subnets list` shows valid ones"
    ),
...
# inline diagnostic stays terse; `subtensor explain <code>` prints these.
...
EXPLANATIONS: dict[ErrorCode, str] = {
    ErrorCode.INSUFFICIENT_BALANCE: (
        ...
        "\n"
        "check the account with `subtensor wallet balance`, then reduce the amount or fund the "
        "coldkey. for stake operations, `subtensor stake list` shows what is actually "
        "staked per subnet."
    ),
    ErrorCode.INSUFFICIENT_LIQUIDITY: (
    ...
    ErrorCode.NOT_REGISTERED: (
        ...
        "`subtensor subnets register --netuid N`, or check where it is registered with "
        "`subtensor view` commands."
    ),
    ErrorCode.NOT_AUTHORIZED: (
    ...
    ErrorCode.SUBNET_NOT_EXISTS: (
        ...
        "exist on testnet. `subtensor subnets list` shows the valid netuids for the "
    ...
    ErrorCode.INVALID_ARGUMENT: (
        ...
        "into its pool.\n"
        "\n"
        "check the argument values against the operation's schema (`subtensor tools` "
