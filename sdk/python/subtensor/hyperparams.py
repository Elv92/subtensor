...
HYPERPARAMS: dict[str, Hyperparam] = {
    ...
    "min_allowed_weights": Hyperparam(
        "int",
        "Minimum number of distinct weights a validator must submit in one "
        "set_weights call.",
    ),
    ...
    "network_pow_registration_allowed": Hyperparam(
        "bool",
        "Whether proof-of-work registration is allowed (as opposed to burned "
        "registration only).",
    ),
    "target_regs_per_interval": Hyperparam(
        "int",
        "Registrations per adjustment interval the difficulty/burn controller "
        "steers toward.",
    ),
    "min_burn": Hyperparam(
        "rao",
    ...
    "max_validators": Hyperparam(
        "int",
        "Maximum validator permits: only the top-stake neurons up to this count "
        "may validate.",
    ),
    ...
    "commit_reveal_period": Hyperparam(
        "epochs",
        "Epochs (tempos) between committing a weights hash and revealing the "
        "actual weights.",
    ),
    ...
    "alpha_high": Hyperparam(
        "u16",
        "Upper bound of the liquid-alpha bonds smoothing range. Stored as u16 "
        "(65535 = 1.0).",
    ),
    "alpha_low": Hyperparam(
        "u16",
        "Lower bound of the liquid-alpha bonds smoothing range. Stored as u16 "
        "(65535 = 1.0).",
    ),
